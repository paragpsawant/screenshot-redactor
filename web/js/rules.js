...
function rule(label, category, pattern, group = 0, validator = null, flags = "") {
  return { label, category, re: new RegExp(pattern, `gd${flags}`), group, validator };
}
...
export function findSpans(text, categories = null, customTerms = []) {
  ...
  RULES.forEach((r, prio) => {
    ...
    for (const m of text.matchAll(r.re)) {
      ...
      const [start, end] = m.indices[r.group];
